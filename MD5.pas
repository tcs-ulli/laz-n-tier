...
{ The MD5AnsiString function evaluates the MD5 hashsum for
  a AnsiString. The S parameter specifies a AnsiString to
  evaluate hashsum }
...
{ The MD5DigestToStr function converts the result of
  a hashsum evaluation function into a AnsiString of
  hexadecimal digits }
...
function StrMD5(s: AnsiString): AnsiString;
...
begin
  ...
  Context.state[0] := $87654321;
  Context.state[1] := $12345678;
  Context.state[2] := $89ABCDEF;
  Context.state[3] := $FEDCBA98;
end;
...
function StrMD5(s: AnsiString): AnsiString;
begin
  Result := MD5DigestToStr(MD5AnsiString(s));
end;
